    ...
    return (
        <div onClick={() => setExpanded(!expanded)} className='p-4 max-w-5xl text-sm bg-white border border-gray-200 rounded-lg cursor-pointer'>
            <div className='flex justify-between items-center gap-4'>
                ...
                <button className='bg-[#EFF6FF] border border-[#BFDFE] text-[#1E40AF] px-4 py-1 rounded-full'>{item.type}</button>
            </div>
            {
                expanded && (
                    <div>
                        {item.type === 'image' ? (
                            ...
                            <div className='mt-3 h-ful overflow-y-scroll text-sm text-slate-700'>
